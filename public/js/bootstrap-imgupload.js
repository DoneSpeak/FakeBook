...
(function($) {
    ...
    function showModalAlert(message){
        var imgupload = $('.imgupload').closest('div');
        var modal = imgupload.find(".modal");
        var modalId = modal.attr('id');
        var msgDiv = imgupload.find(".modal-body");
        msgDiv.html(message);
        // alert(modalId);
    ...
    function selectImageFile($button) {
        ...
        $fileTab.css('display','block');
        $('.discrip-img').find('input').val("");
        $('.discrip-img').css('display','block');
        ...
        if (file.size / 1024 > options.maxFileSizeKb)
        {
            ...
            $input.val("");
            showModalAlert("图片最大为" + options.maxFileSizeKb + "KB");
            // $fileTab.append(getHtmlErrorMessage("File size is too large (max " +."));
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            ...
            showModalAlert("图片格式应该为：" + options.allowedFormats.join('/'));
        }
    }
